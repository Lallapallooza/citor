...
TEST(ParallelPoolTsanStress, AllPrimitivesUnderRandomizedParticipants) {
  ...
  for (int iter = 0; iter < kIterations; ++iter) {
    ...
    case 2: {
      // parallelReduce with FixedBlockOrder + Kahan via the KahanReduceHints named hint. The producer
      // joins on every chunk, then reads partials back; TSan would flag any missing happens-before
      // edge between the worker's slot write and the producer's tree combine.
