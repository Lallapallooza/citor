...
// observed.
//
// Skipped when the pool collapses to a single participant: the inline-fallback path runs the body
// once over the full range, so a token requested mid-body cannot abort any subsequent chunk (there
// are none). The chunk-boundary cancellation contract has no observable surface in that mode.
