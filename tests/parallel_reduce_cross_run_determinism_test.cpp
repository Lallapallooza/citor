...
// Build the same data buffer every time from a fixed RNG seed. Two process invocations that hit
// this function with the same `seed` must observe byte-identical buffers; this is the foundation
// for the cross-run determinism gate -- the data feed is process-independent so the only
// remaining source of variation is the reduction engine itself.
...
// Cross-run determinism: a fixed seed feeds a deterministic data buffer; for each `nJobs` in
// {1,2,4,8,16}, repeat the reduction 10 times and assert the f64 bit pattern is stable. Within a
// single process invocation the result must be byte-identical across repeats; across two process
// invocations the same property holds because (a) the seed produces the same buffer, (b) the
// reduction engine's chunk shape is `n`-determined, and (c) the chunk-id pairwise tree's combine
// order is fixed. The within-nJobs repeats here therefore validate the cross-run invariant up to
// the inputs.
  ...
  for (const std::size_t j :
    ...
    for (int rep = 1; rep < kRepeats; ++rep) {
      ...
      ASSERT_EQ(std::bit_cast<std::uint64_t>(reference), std::bit_cast<std::uint64_t>(current))
          << "FixedBlockOrder cross-run repeatability broke at j=" << j << " rep=" << rep;
    }
  }
}

// Cross-run determinism for the Kahan path: the same data and seed produce the same Kahan tree
// result on every invocation at every `nJobs`.
...
// Cross-`nJobs` bit identity at fixed seed: at the same input, every `nJobs` produces the same
// result. This complements the existing `parallel_reduce_determinism_test.cpp` cross-`nJobs`
// gate by sourcing the inputs from a fixed RNG seed -- the gate is unchanged but the data feed
...
TEST(ParallelReduceCrossRunDeterminism, KahanCompensatedBitIdenticalAcrossJobs) {
  constexpr std::size_t kN = 8192;
