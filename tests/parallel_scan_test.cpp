...
// Pass-1 throw inside a worker leaves it stamped at pass2Stamp; the producer's seq-reduce then
// runs unconditionally and may invoke `prefix` on partials that were never written. The producer
// must not call `prefix` after a pass-1 failure -- the call should propagate the captured
// exception without any further user-callable being invoked.
TEST(ParallelScan, PrefixNotCalledAfterPass1Failure) {
  ThreadPool pool(4);
  if (pool.participants() < 2U) {
  ...
  auto body = [](std::size_t slot, std::size_t /*lo*/, std::size_t /*hi*/, std::int64_t /*init*/,
  ...
  ASSERT_EQ(fut.wait_for(std::chrono::seconds(2)), std::future_status::ready)
      << "parallelScan deadlocked when prefix would otherwise have run on partial state";
  fut.get();
  EXPECT_EQ(prefixCalls.load(), 0);
}

// User `prefix` throwing during the producer's sequential reduce must not deadlock workers
// that are still spinning on `prefixesPublished`. The producer must publish the flag before
// unwinding so peers can proceed (or short-circuit on the captured exception).
TEST(ParallelScan, PrefixThrowDoesNotDeadlock) {
  ThreadPool pool(4);
  if (pool.participants() < 2U) {
    GTEST_SKIP() << "test requires multi-participant pool";
  }
  ...
  auto body = [](std::size_t /*slot*/, std::size_t /*lo*/, std::size_t /*hi*/,
                 std::int64_t /*init*/, std::int64_t * /*out*/) -> std::int64_t { return 1; };
  ...
  ASSERT_EQ(fut.wait_for(std::chrono::seconds(2)), std::future_status::ready)
      << "parallelScan deadlocked when user prefix threw during producer reduce";
  fut.get();
}
