...
// Dynamic-chain mode opts out of same-chunk pipelining: each globally synchronized stage is split
// into chunks that participants claim from a counter. The global barrier must still publish all
// stage-0 writes before stage 1 can observe them.
TEST(ParallelChain, DynamicGlobalStagesPartitionExactly) {
  ThreadPool pool(4);
  ...
  pool.parallelChain<DynamicChainTestHints>(
      ...
      globalStage("dynamic-2", [&](std::size_t /*stageIdx*/, std::uint32_t slot, std::size_t lo,
                                   std::size_t hi) {
        if (slot >= participants) {
          badSlot.fetch_add(1, std::memory_order_acq_rel);
        }
  ...
  for (std::size_t i = 0; i < kN; ++i) {
    EXPECT_EQ(stage0Coverage[i].load(std::memory_order_acquire), 1U) << "stage0 i=" << i;
    EXPECT_EQ(stage1Coverage[i].load(std::memory_order_acquire), 1U) << "stage1 i=" << i;
    EXPECT_EQ(stage2Coverage[i].load(std::memory_order_acquire), 1U) << "stage2 i=" << i;
  }
}
