...
Per-family geomean speedups vs ten competitor pools (BS, dp, task, riften, oneTBB, Taskflow, Eigen, OpenMP, Leopard, dispenso). Charts are regenerated by `python -m tools.plot_bench`; the underlying numbers live in `bench_out/<host>/<sha>/results.json`.
