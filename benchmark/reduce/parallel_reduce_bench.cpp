...
//   - citor pool              -> `parallelReduce<citor::HintsDefaults>`
//   (native).
//   - BS::light_thread_pool   -> two-wave: per-block partials via
//   `submit_blocks`,
//                                serial merge.
...
//   - oneTBB                  -> `tbb::parallel_reduce` via
//   `task_arena::execute`.
...
template <class T>
[[nodiscard]] ReduceData<T> buildData() {
  ReduceData<T> d;
  d.in.assign(kN, T{0});
...
template <class T>
[[nodiscard]] T computeReference(const ReduceData<T> &d) noexcept {
  T total{0};
  for (T v : d.in) {
    total += v;
  }
...
/// abort message names the offending row, the expected sum, and the observed
/// sum.
template <class T>
inline void reportReduceMismatch(const char *name, T reference, T value) {
  if constexpr (std::is_floating_point_v<T>) {
    std::fprintf(stderr, "[%s] reduce mismatch: expected=%.17g actual=%.17g\n",
                 name, static_cast<double>(reference),
                 static_cast<double>(value));
  } else {
    std::fprintf(stderr, "[%s] reduce mismatch: expected=%lld actual=%lld\n",
                 name, static_cast<long long>(reference),
...
template <class T>
[[nodiscard]] T partialSumPlain(const std::vector<T> &in, std::size_t lo,
...
[[nodiscard]] inline double partialSumKahan(const std::vector<double> &in,
                                            std::size_t lo,
                                            std::size_t hi) noexcept {
  double s = 0.0;
  double c = 0.0;
...
[[nodiscard]] BenchRow measureCitorPlain(std::size_t participants,
  ...
  return measureLoop<std::int64_t>(
      ...
      [&] {
        return pool.parallelReduce<citor::HintsDefaults>(
            ...
            [&d](std::size_t lo, std::size_t hi) {
              return partialSumPlain(d.in, lo, hi);
            },
...
[[nodiscard]] BenchRow measureCitorKahan(std::size_t participants,
  ...
  return measureLoop<double>(
      "citor::ThreadPool::parallelReduce_kahan", cal,
      [&] {
        return pool.parallelReduce<citor::KahanReduceHints>(
            std::size_t{0}, kN, 0.0,
            [&d](std::size_t lo, std::size_t hi) {
              return partialSumKahan(d.in, lo, hi);
            },
            std::plus<double>{});
      },
      reference);
...
[[nodiscard]] T runBsTwoWave(BS::light_thread_pool &pool,
                             const std::vector<T> &in, std::size_t blocks,
                             Kernel kernel) {
...
[[nodiscard]] BenchRow measureBsPlain(std::size_t participants,
                                      const CyclesPerNanosecond &cal) {
  BS::light_thread_pool pool(participants);
  ReduceData<std::int64_t> d = buildData<std::int64_t>();
  const std::int64_t reference = computeReference(d);
  return measureLoop<std::int64_t>(
      "BS::thread_pool::reduce_two_wave", cal,
      [&] {
        return runBsTwoWave<std::int64_t>(pool, d.in, participants,
                                          partialSumPlain<std::int64_t>);
      },
      reference);
}

[[nodiscard]] BenchRow measureBsKahan(std::size_t participants,
                                      const CyclesPerNanosecond &cal) {
  BS::light_thread_pool pool(participants);
  ReduceData<double> d = buildData<double>();
  const double reference = computeReference(d);
  return measureLoop<double>(
      "BS::thread_pool::reduce_kahan_two_wave", cal,
      [&] {
...
[[nodiscard]] T runFutureTwoWave(Pool &pool, const std::vector<T> &in,
  ...
  for (std::size_t b = 0; b < blocks; ++b) {
    ...
    futs.emplace_back(
        enqueue(pool, [&in, lo, hi, kernel] { return kernel(in, lo, hi); }));
  }
  T total{0};
  for (auto &f : futs) {
    total += f.get();
...
[[nodiscard]] BenchRow measureDpPlain(std::size_t participants,
                                      const CyclesPerNanosecond &cal) {
  dp::thread_pool<> pool(static_cast<unsigned int>(participants));
  ReduceData<std::int64_t> d = buildData<std::int64_t>();
  const std::int64_t reference = computeReference(d);
  return measureLoop<std::int64_t>(
      "dp::thread_pool::reduce_two_wave", cal,
      [&] {
        return runFutureTwoWave<std::int64_t>(
            pool, d.in, participants,
            [](dp::thread_pool<> &p, auto fn) {
              return p.enqueue(std::move(fn));
            },
            partialSumPlain<std::int64_t>);
      },
      reference);
...
[[nodiscard]] BenchRow measureDpKahan(std::size_t participants,
                                      const CyclesPerNanosecond &cal) {
  dp::thread_pool<> pool(static_cast<unsigned int>(participants));
  ReduceData<double> d = buildData<double>();
  const double reference = computeReference(d);
  return measureLoop<double>(
      "dp::thread_pool::reduce_kahan_two_wave", cal,
      [&] {
...
[[nodiscard]] BenchRow measureTaskPlain(std::size_t participants,
                                        const CyclesPerNanosecond &cal) {
  ...
  const std::int64_t reference = computeReference(d);
  return measureLoop<std::int64_t>(
      "task_thread_pool::reduce_two_wave", cal,
      [&] {
        return runFutureTwoWave<std::int64_t>(
            pool, d.in, participants,
...
[[nodiscard]] BenchRow measureTaskKahan(std::size_t participants,
                                        const CyclesPerNanosecond &cal) {
  ::task_thread_pool::task_thread_pool pool(
      static_cast<unsigned int>(participants));
  ReduceData<double> d = buildData<double>();
  const double reference = computeReference(d);
  return measureLoop<double>(
      "task_thread_pool::reduce_kahan_two_wave", cal,
      [&] {
        return runFutureTwoWave<double>(
            pool, d.in, participants,
            [](::task_thread_pool::task_thread_pool &p, auto fn) {
...
[[nodiscard]] BenchRow measureRiftenPlain(std::size_t participants,
  ...
  const std::int64_t reference = computeReference(d);
  return measureLoop<std::int64_t>(
      "riften::Thiefpool::reduce_two_wave", cal,
      [&] {
        return runFutureTwoWave<std::int64_t>(
            pool, d.in, participants,
            [](riften::Thiefpool &p, auto fn) {
              return p.enqueue(std::move(fn));
            },
            partialSumPlain<std::int64_t>);
      },
...
[[nodiscard]] BenchRow measureRiftenKahan(std::size_t participants,
  ...
  const double reference = computeReference(d);
  return measureLoop<double>(
      "riften::Thiefpool::reduce_kahan_two_wave", cal,
      [&] {
        return runFutureTwoWave<double>(
            pool, d.in, participants,
            [](riften::Thiefpool &p, auto fn) {
...
#ifdef CITOR_BENCH_HAS_TBB
[[nodiscard]] BenchRow measureTbbPlain(std::size_t participants,
                                       const CyclesPerNanosecond &cal) {
...
[[nodiscard]] BenchRow measureTbbKahan(std::size_t participants,
                                       const CyclesPerNanosecond &cal) {
  auto arena = CompetitorTraits<::tbb::task_arena>::make(participants);
  ReduceData<double> d = buildData<double>();
  const double reference = computeReference(d);
  return measureLoop<double>(
      "oneTBB::parallel_reduce_kahan", cal,
      [&] {
...
#ifdef CITOR_BENCH_HAS_TASKFLOW
template <class T, class Kernel>
[[nodiscard]] T runTaskflowTwoWave(::tf::Executor &exec,
                                   const std::vector<T> &in, std::size_t blocks,
                                   Kernel kernel) {
  std::vector<T> partials(blocks, T{0});
  ::tf::Taskflow flow;
  flow.for_each_index(
      std::size_t{0}, blocks, std::size_t{1},
...
[[nodiscard]] BenchRow measureTaskflowKahan(std::size_t participants,
  ...
  const double reference = computeReference(d);
  return measureLoop<double>(
      "Taskflow::reduce_kahan_two_wave", cal,
      [&] {
        return runTaskflowTwoWave<double>(*exec, d.in, participants,
                                          partialSumKahan);
      },
      reference);
}
...
#ifdef CITOR_BENCH_HAS_EIGEN_THREADPOOL
template <class T, class Kernel>
[[nodiscard]] T runEigenTwoWave(::Eigen::ThreadPool &pool,
                                const std::vector<T> &in, std::size_t blocks,
                                Kernel kernel) {
...
[[nodiscard]] BenchRow measureEigenKahan(std::size_t participants,
  ...
  const double reference = computeReference(d);
  return measureLoop<double>(
      "Eigen::ThreadPool::reduce_kahan_two_wave", cal,
      [&] {
        return runEigenTwoWave<double>(*pool, d.in, participants,
                                       partialSumKahan);
      },
      reference);
}
...
template <class T, class Kernel>
[[nodiscard]] T runLeopardTwoWave(hmthrp::ThreadPool &pool,
                                  const std::vector<T> &in, std::size_t blocks,
                                  Kernel kernel) {
  // Direct dispatch -- one block per partial -- so the kernel's per-block
  // result lands at the matching `partials[b]`. Bypassing `Traits::parallelFor`
  ...
  for (std::size_t b = 0; b < blocks; ++b) {
    ...
    futures.emplace_back(
        pool.dispatch(false, [b, lo, hi, &in, &partials, kernel]() {
          partials[b] = kernel(in, lo, hi);
        }));
  }
  for (auto &f : futures) {
...
[[nodiscard]] BenchRow measureLeopardKahan(std::size_t participants,
  ...
  const double reference = computeReference(d);
  return measureLoop<double>(
      "Leopard::reduce_kahan_two_wave", cal,
      [&] {
        return runLeopardTwoWave<double>(*pool, d.in, participants,
                                         partialSumKahan);
      },
      reference);
}
...
template <class T, class Kernel>
[[nodiscard]] T runDispensoTwoWave(dispenso::ThreadPool &pool,
                                   const std::vector<T> &in, std::size_t blocks,
                                   Kernel kernel) {
...
[[nodiscard]] BenchRow measureDispensoKahan(std::size_t participants,
  ...
  const double reference = computeReference(d);
  return measureLoop<double>(
      "dispenso::reduce_kahan_two_wave", cal,
      [&] {
        return runDispensoTwoWave<double>(*pool, d.in, participants,
                                          partialSumKahan);
      },
      reference);
}
...
[[nodiscard]] BenchRow measureOpenMpKahan(std::size_t participants,
  ...
  // computing per-thread Kahan sums into a per-thread partials array and
  // merging serially, matching the shape of the other emulations.
  ReduceData<double> d = buildData<double>();
  const double reference = computeReference(d);
  const auto threads = static_cast<int>(participants);
  return measureLoop<double>(
      "OpenMP::reduce_kahan_two_wave", cal,
      [&] {
...
#pragma omp parallel for num_threads(threads) schedule(static)
        for (std::ptrdiff_t b = 0; b < blocks; ++b) {
          ...
          const std::size_t blockSize =
              (kN + static_cast<std::size_t>(threads) - 1) /
